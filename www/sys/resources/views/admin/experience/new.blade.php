                        ...
                        <div class="row ">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <select class="form-control @error('exp_month_start') is-invalid @enderror" name="exp_month_start" id="exp_month_start">
                                                @for ($i = 0; $i <= 11; $i++)
                                                    <option {{ (old('exp_month_start') == $i) ? 'selected' : '' }} value='{{ $i }}'>{{ Tools::months($i) }}</option>
                                                @endfor
                                            </select>
                                        </div>
                                        <div class="col-md-6">
                                            <select class="form-control @error('exp_year_start') is-invalid @enderror" name="exp_year_start" id="exp_year_start">
                                                @for ($j = date('Y'); $j >= 1800; $j--)
                                                    <option {{ (old('exp_year_start') == $j) ? 'selected' : '' }} value="{{ $j }}">{{ $j }}</option>
                                                @endfor
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <select class="form-control @error('exp_month_finish') is-invalid @enderror date_month_finish" name="exp_month_finish" id="exp_month_finish" {{ (old('exp_current')) ? 'disabled' : '' }}>
                                                @for ($i = 0; $i <= 11; $i++)
                                                    <option {{ (old('exp_month_finish') == $i) ? 'selected' : '' }} value='{{ $i }}'>{{ Tools::months($i) }}</option>
                                                @endfor
                                            </select>
                                        </div>
                                        <div class="col-md-6">
                                            <select class="form-control @error('exp_year_finish') is-invalid @enderror date_year_finish" name="exp_year_finish" id="exp_year_finish" {{ (old('exp_current')) ? 'disabled' : '' }}>
                                                @for ($j = date('Y'); $j >= 1800; $j--)
                                                    <option {{ (old('exp_year_finish') == $j) ? 'selected' : '' }} value="{{ $j }}">{{ $j }}</option>
                                                @endfor
                                            </select>
                                        </div>
                                        <div class="col-md-12">
                                            <label for="">At present</label>
                                            <input class="activeLastDate" name="exp_current" id="exp_current" type="checkbox" {{ (old('exp_current')) ? 'checked' : '' }} >
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea id="exp_description" rows="5" class="form-control @error('exp_description') is-invalid @enderror" name="exp_description">{{ old('exp_description') }}</textarea>
